...
namespace ocs2::ballbot {
    ad_vector_t BallbotSystemDynamics::systemFlowMap(ad_scalar_t time, const ad_vector_t &state,
        ...
        S_transposed(0, 0) =
                -(sqrt_2 * (cyaw * sroll - croll * spitch * syaw)) / (2 * wheelRadius_) - (sqrt_2 * cpitch * syaw) / (
                    2 * wheelRadius_);
        ...
        S_transposed(1, 1) =
                -(sqrt_2 * (c5)) / (2.0 * wheelRadius_) - (c6) / (4.0 * wheelRadius_) - (sqrt_2 * sqrt_3 * (c4)) / (
                    4.0 * wheelRadius_);
        S_transposed(1, 2) =
                (sqrt_2 * sqrt_3 * (c4)) / (4.0 * wheelRadius_) - (c6) / (4.0 * wheelRadius_) - (sqrt_2 * (c5)) / (
                    2.0 * wheelRadius_);

        S_transposed(2, 0) = -(sqrt_2 * ballRadius_ * (spitch + cpitch * croll)) / (2 * wheelRadius_);
        S_transposed(2, 1) = (sqrt_2 * ballRadius_ * (spitch - 2 * cpitch * croll + sqrt_3 * cpitch * sroll)) / (
                                 4 * wheelRadius_);
        S_transposed(2, 2) = -(sqrt_2 * ballRadius_ * (2 * cpitch * croll - spitch + sqrt_3 * cpitch * sroll)) / (
                                 4 * wheelRadius_);
