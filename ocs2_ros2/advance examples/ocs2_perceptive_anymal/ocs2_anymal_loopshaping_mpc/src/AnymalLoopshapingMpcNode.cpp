...
int main(int argc, char* argv[]) {
  ...
  rclcpp::Node::SharedPtr node =
      rclcpp::Node::make_shared("anymal_loopshaping_mpc");
  ...
  switch (anymalInterface->modelSettings().algorithm_) {
    case switched_model::Algorithm::DDP: {
      const auto ddpSettings = ocs2::ddp::loadSettings(
          anymal::getTaskFilePathLoopshaping(configName));
    ...
    case switched_model::Algorithm::SQP: {
      const auto sqpSettings = ocs2::sqp::loadSettings(
          anymal::getConfigFolderLoopshaping(configName) +
